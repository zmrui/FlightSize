...
void tcp_enter_loss(struct sock *sk)
{
	...
	// printk("[tcp_enter_loss] cwnd before=%u",tp->snd_cwnd);
	tp->snd_cwnd	   = tcp_packets_in_flight(tp) + 1;
	// printk("[tcp_enter_loss] cwnd after=%u=fightsize+1",tp->snd_cwnd);
	tp->snd_cwnd_cnt   = 0;
	tp->snd_cwnd_stamp = tcp_jiffies32;
...
void tcp_cwnd_reduction(struct sock *sk, int newly_acked_sacked, int newly_lost, int flag)
{
	...
	// printk("[tcp_cwnd_reduction] delta:%d=snd_ssthresh:%u-flightsize",delta,tp->snd_ssthresh);
	...
	// printk("[tcp_cwnd_reduction] snd_cwnd:%u=sndcnt:%d+flightsize",tp->snd_cwnd,sndcnt);
}
...
static int tcp_ack(struct sock *sk, const struct sk_buff *skb, int flag)
{
	...
	// printk("[tcp_ack] rs.prior_in_flight=%u",rs.prior_in_flight);
