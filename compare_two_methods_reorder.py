...
def Reorder_Compare_two_methods(cca,rtt, bw,  reorder_percentage_in_10000,reorder_distance,loss=0,nameprefix="", keep=1):

    reorder_probility=reorder_percentage_in_10000/100.0
    ...
    reordercmd = f'sudo tc qdisc change dev h3-eth0 parent 5:1 handle 10:0 netem delay 100ms reorder {reorder_tc_prob}% 50%'
    ...
    mn_net.start_iperf_time_json(durition_time=5)
...
if __name__ == "__main__":
    for cca in ['cubic','bbr']:
        for rtt in [1]:
            for bw in [10]:
                for rate in [1,10,50,100,500,1000,2000,8000]:
                    for run in range(10):
                        print("++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
                        print(datetime.now())
                        try:
                            Reorder_Compare_two_methods(cca=cca,rtt=rtt,bw=bw,reorder_percentage_in_10000=rate,reorder_distance=0.5)
