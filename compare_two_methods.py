...
def Compare_two_methods(cca,rtt, bw, loss,nameprefix="", keep=1):
    ...
    resfolder = os.path.join(Mininet_testbed.utils.config.MN_TESTBED_DIR,'noloss')
    ...
    fslin_impact, fs1_impact, fs2_impact = fc.diff_time()
    ...
    with open(os.path.join(fc.folder,'result.txt'),'w') as f:
        f.write(f'average_E_tp:{average_E_tp} average_E_pk"{average_E_pk}\nFS Lin:{fslin_impact} FS Online:{fs1_impact} FS Offiline:{fs2_impact}')
